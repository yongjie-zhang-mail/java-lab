...
import com.yj.lab.spring.model.excel.ExcelTumorAssessmentTargetLesions;
...
public class MedicalApp {
    ...
    public void extractMedicalData() {
        ...
        String fileName = TestFileUtil.getPath() + "excel" + File.separator + "Data listing for medical review_FN1502_dc_01SEP22.xlsx";

        DataListener<ExcelTumorAssessmentTargetLesions> dl1 = new DataListener<>();
//        DataListener<DemoData> dl2 = new DataListener<>();

        try (ExcelReader excelReader = EasyExcel.read(fileName).build()) {
            // 这里为了简单 所以注册了 同样的head 和Listener 自己使用功能必须不同的Listener
            ReadSheet readSheet1 = EasyExcel.readSheet("肿瘤评估-（靶+非靶+新+整体）")
                    .head(ExcelTumorAssessmentTargetLesions.class).registerReadListener(dl1).build();
//            ReadSheet readSheet2 = EasyExcel.readSheet(1).head(DemoData.class).registerReadListener(dl2).build();
            // 这里注意 一定要把sheet1 sheet2 一起传进去，不然有个问题就是03版的excel 会读取多次，浪费性能
            excelReader.read(readSheet1);
        }

        List<ExcelTumorAssessmentTargetLesions> sd1 = dl1.getSheetData();
